...
(function () {
  ...
  var PROFILE_FIELD_IDS = ['company', 'industry', 'gtmteam', 'revenue', 'location'];

  // All 5 profile fields are required before continuing past Step 2.
  // Validates on Next click (not on initial render, so a returning user
  // isn't greeted with errors) and live-clears each field's invalid mark
  // as it's filled in, matching the pillar-selection note just above it.
  function validateProfileFields() {
    ...
    var note = document.getElementById('profileFieldsNote');
    if (!allValid) {
      note.textContent = 'Please fill in all fields above before continuing.';
  ...
  function onProfileFieldChange(e) {
    ...
    if (e.target.value && e.target.value.trim() !== '') {
      ...
      var stillMissing = PROFILE_FIELD_IDS.some(function (id) {
        return document.getElementById(id).classList.contains('invalid');
      });
      if (!stillMissing) document.getElementById('profileFieldsNote').textContent = '';
    }
  }
  ...
  function renderResults() {
    ...
    // Lead capture, post 1 of 2: the assessment, every time Results renders
    // (an unchanged revisit posts nothing -- see capture.js).
    if (window.VLG_CAPTURE_CLIENT) window.VLG_CAPTURE_CLIENT.captureAssessment(state, result, CALC.CAP_KEYS);
  ...
  // The report itself is emailed by the VlgMail stack (mailer/handler.py) via
  // report-client.js, fired from submitGate's finish() below. It is
  // fire-and-forget: the confirmation never waits on it, and a failure lands
  // in CloudWatch, not in front of the prospect.
  ...
  function submitGate(e) {
    ...
    // Called once the request is accepted (HubSpot succeeded, or HubSpot is
    // not configured yet). Captures the lead into the Sheet row, fires the
    // report email, and confirms -- without waiting on either request. Both
    // are fire-and-forget by design (capture.js / report-client.js).
    function finish() {
      var capture = window.VLG_CAPTURE_CLIENT;
      var reportClient = window.VLG_REPORT_CLIENT;
      var responseId = capture
        ? capture.captureLead(contact, state, result, CALC.CAP_KEYS, Date.now() - modalOpenedAt)
        : '';
      if (reportClient) reportClient.send(contact, state.profile, state.toggles, state.ratings, responseId);
      done();
    }

    if (!client || !client.isEnabled()) { finish(); return false; }
  ...
  function startOver() {
    ...
    if (window.VLG_CAPTURE_CLIENT) window.VLG_CAPTURE_CLIENT.newTurn(false);
    render();
  }
