...
| O-09 | Static site: choose and set up hosting (AWS S3 + CloudFront is the verified live pattern at massgroup.geniusdrive.com; see WEB PROJECT template/modules/hosting_static_s3_cloudfront.md) | Open |
| O-10 | Static site: build data capture (Google Sheets via Apps Script Web App) -- explicitly deferred by Ben until after the app itself is tested | Open |
| O-11 | Static site: wire up "Get My Report" (currently an inert placeholder matching the locked mockup's canned confirmation) once data capture and report generation are scoped | Mostly done (Session 12) -- lead capture to HubSpot (Session 11) plus a real report-generation call now both fire from the same submit; report download triggers client-side once static-site/report-config.js's apiUrl is filled in (blank/off by default). Remaining: Ben needs to actually deploy the Lambda + API Gateway endpoint (output_report/DATA_CONTRACT.md has the exact commands) and fill in that URL -- the Docker image itself is written but not yet built/tested anywhere (network policy blocked it this session) |
