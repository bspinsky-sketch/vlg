...
| 2026-09-23 22:39 EDT | 90f5b46 | Session 18 final commit, pushed by Ben from his own machine (af9ab2a..90f5b46, 4 files: .gitignore, PROJECT_STATE.md, SESSION_LOG.md, static-site/capture-config.js with the live Apps Script endpoint). Repo now matches everything deployed. LF->CRLF warnings on add are line-ending noise only. This registry row lands in the next commit. |
| 2026-09-23 23:00 EDT | `cdk deploy VlgSite` (app fixes) | VlgSite redeployed by Ben (102.22s) with 3 Results/modal fixes Ben approved: (1) Results headline now dynamic 'From <band> to Value as a System.' (bottom band -> 'Value Without a Narrative') + static lede, mirroring report 08-where-you-stand; (2) 'Build your system' headline (no period) + subhead mirroring report 09-next-moves; (3) Get My Report form pre-fills Company from the Profile, editable, never overwrites a visitor's own edit. Playwright-verified (bottom/mixed/top bands, prefill open/edit/reopen/profile-change). Open question to Ben: top band reads 'From Value as a System to Value as a System.' in both site and report. Commit pending. |
