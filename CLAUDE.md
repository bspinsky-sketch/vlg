...
| Decision | Rationale |
|----------|-----------|
| Report template copy/layout pass -- 7 items (2026-09-23, Session 17) | Slide 2/7/8 headline+subhead rewording, slide 7 legend repositioned + reworded to match the app's own copy, slides 10-13 "(N of 4)" pagination, slides 6/7 sentence case, slides 9-15 no terminal period, slide 16 hyperlink retargeted. Slide 8's headline now dynamically names the visitor's actual maturity band against the top band ("Value as a System" -- a real data.json label, not invented copy), with the bottom band special-cased to avoid a "From No..." grammar break. See SESSION_LOG.md Session 17 for the full negotiation history and a CSS `:first-of-type` bug caught and fixed (matches by tag name, not class -- silently matched nothing). |
